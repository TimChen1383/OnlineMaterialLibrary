# OnlineMaterialLibrary (WIP. Temporary name)
Due to the shader language and the node based design of the game engine, materials are hard to share across multiple platforms. Which means you can't see it on web browser, hard to shader with other people an the material can only be use in single platform like Unreal Engine.

This platform is based on Slang(https://shader-slang.org/), which supports multiple backend targets for portable code deployment across diverse APIs and platforms.
It cludes features below:
- Code based. Based on the industry standard shader language Slang
- Write once and server will compile to code for multiple platforms
- Can be converted and demostrate in realtime through Web Browser
- Can be converted and export to game engine like Unreal Engine and Unity
- Storing ready to use materials as library on the platform
- Easy to share with other people

Compiler
<img width="1265" height="710" alt="Slang" src="https://github.com/user-attachments/assets/6e1983ae-216f-45b2-92b0-3ac921fd6d23" />

Shader Library
<img width="1902" height="886" alt="ShaderLibrary" src="https://github.com/user-attachments/assets/ab105995-6fbe-41fe-afdd-5eb89d892e5e" />

Code Editor
<img width="1919" height="902" alt="Editor" src="https://github.com/user-attachments/assets/c0e41219-45eb-4586-aad1-544fa49841ab" />

Cross Platform Export
<img width="1917" height="913" alt="Export" src="https://github.com/user-attachments/assets/e757cd8f-1764-40e0-af2a-3411b7146229" />
